...
public class ManageMapServlet extends BaseServlet {
	...
	public ST processBody(HttpServletRequest req,
		...
		try {
			...
			File[] files = ServerUtils.getMaps(serverId);	
...
//				if (files[i].getName().equals(server.getMapName())) continue; //remove the selected map
	...
	private static String getFileExtention(String url) {
